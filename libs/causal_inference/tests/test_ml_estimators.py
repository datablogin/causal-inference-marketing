...
class TestCrossValidationPerformance:
    ...
    def test_tmle_vs_doubly_robust_ml_comparison(self, medium_confounded_data):
        ...
        # Note: With reduced sample sizes for CI speed, statistical variance increases
        assert (
            abs(tmle_effect.ate - true_ate) < 20.0
        )  # Extremely relaxed tolerance for very small samples
        assert (
            abs(drml_effect.ate - true_ate) < 20.0
        )  # Extremely relaxed tolerance for very small samples
        ...
        assert (
            abs(tmle_effect.ate - drml_effect.ate) < 20.0
        )  # Relaxed for small sample variance

...
class TestSurvivalOutcomeData:
    ...
    def test_survival_data_validation(self):
        ...
        with pytest.raises(ValueError, match="Times and events must have the same length"):
    ...
    def test_survival_data_with_competing_risks(self):
        ...
        event_types = np.array([1, 2, 0, 1, 2])  # 0=censored, 1=event of interest, 2=competing
...
class TestSurvivalGComputationEstimator:
    ...
    def test_gcomputation_fitting_and_estimation(self):
        ...
        # Generate test data
        treatment, outcome, covariates = TestSurvivalDataGeneration.generate_survival_data(
...
class TestSurvivalEstimatorEdgeCases:
    ...
    def test_insufficient_events_error(self):
        ...
        with pytest.raises(Exception):  # Should raise EstimationError about insufficient events
...
class TestSurvivalEstimatorIntegration:
    ...
    def test_estimator_consistency(self):
        ...
        aipw = SurvivalAIPWEstimator(
            survival_model="cox", propensity_model="logistic", 
            time_horizon=40, random_state=42
        )

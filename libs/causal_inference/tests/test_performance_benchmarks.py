...
class TestEstimatorPerformance:
    ...
    @pytest.mark.slow
    def test_g_computation_performance_medium(self, benchmark, medium_sample_size, random_state):
        ...
        outcome = X @ outcome_weights + 2 * treatment + np.random.normal(0, 1, medium_sample_size)
        ...
        covariate_data = CovariateData(values=pd.DataFrame(X, columns=[f"X{i}" for i in range(n_features)]))
    ...
    def test_performance_scaling(self, performance_benchmark_data):
        ...
        for estimator_name, result in results.items():
            ...
            assert result["time"] < 30.0, f"{estimator_name} took too long: {result['time']:.2f}s"
    ...
    def test_memory_usage_large_dataset(self, large_sample_size, random_state):
        ...
        estimator = GComputationEstimator(bootstrap_samples=10)  # Reduced bootstrap for memory
        estimator.fit(treatment_data, outcome_data, covariate_data)
        effect = estimator.estimate_ate()
...
class TestDataProcessingPerformance:
    ...
    def test_synthetic_data_generation_performance(self, benchmark, large_sample_size, random_state):
    ...
    def test_missing_data_handling_performance(self, benchmark, performance_benchmark_data):
...
class TestDiagnosticsPerformance:
    ...
    def test_balance_diagnostics_performance(self, benchmark, performance_benchmark_data):
    ...
    def test_overlap_diagnostics_performance(self, benchmark, performance_benchmark_data):
...
class TestPerformanceRegression:
    ...
    def test_g_computation_performance_regression(self, medium_sample_size, random_state):
        ...
        outcome = np.sum(X, axis=1) + 2 * treatment + np.random.normal(0, 1, medium_sample_size)

        from causal_inference.core.base import CovariateData, OutcomeData, TreatmentData

        treatment_data = TreatmentData(values=treatment, treatment_type="binary")
        outcome_data = OutcomeData(values=outcome, outcome_type="continuous")
        covariate_data = CovariateData(values=pd.DataFrame(X, columns=[f"X{i}" for i in range(5)]))
    ...
    def test_performance_vs_feature_count(self, n_features, small_sample_size, random_state):
        ...
        outcome = np.sum(X[:, :min(3, n_features)], axis=1) + treatment + np.random.normal(0, 0.5, small_sample_size)
        ...
        estimator = GComputationEstimator(bootstrap_samples=10)
        estimator.fit(treatment_data, outcome_data, covariate_data)
        effect = estimator.estimate_ate()
        ...
        assert elapsed_time < max_expected_time, f"Too slow for {n_features} features: {elapsed_time:.2f}s"
        assert np.isfinite(effect.ate)

...
class SurvivalAIPWEstimator(SurvivalGComputationEstimator, SurvivalIPWEstimator):
    ...
    def _fit_implementation(
        ...
        if covariates is None:
            raise EstimationError("Covariates are required for AIPW survival estimation")
            
        # Fit outcome model (G-computation component)
        SurvivalGComputationEstimator._fit_implementation(self, treatment, outcome, covariates)
    ...
    def _compute_aipw_survival_curve(
        ...
        outcome_curve = self._predict_survival_curve(treatment_value=treatment_value, times=times)
        ...
        n_treated = np.sum(treatment_mask)
        n_total = len(self.treatment_data.values)
        
        # Weight the combination based on effective sample size
        effective_n = np.sum(observed_weights)
        model_weight = 0.5  # Could be made adaptive
        observed_weight = 1 - model_weight
        
        aipw_probs = (
            model_weight * outcome_curve['survival_prob'].values +
            observed_weight * np.array(observed_probs)
        )
        
        return pd.DataFrame({
            'timeline': times,
            'survival_prob': aipw_probs
        })
    ...
    def estimate_survival_curves(self) -> dict[str, pd.DataFrame]:
        ...
        treated_curve = self._compute_aipw_survival_curve(treatment_value=1, times=times)
        control_curve = self._compute_aipw_survival_curve(treatment_value=0, times=times)
    ...
    def compare_components(self) -> dict[str, Any]:
        ...
        if self.time_horizon is not None:
            comparison['g_computation']['rmst_treated'] = restricted_mean_survival_time(
                gcomp_treated['timeline'], gcomp_treated['survival_prob'], t=self.time_horizon
            )
            comparison['g_computation']['rmst_control'] = restricted_mean_survival_time(
                gcomp_control['timeline'], gcomp_control['survival_prob'], t=self.time_horizon
            )
            
            comparison['ipw']['rmst_treated'] = restricted_mean_survival_time(
                ipw_curves['treated']['timeline'], ipw_curves['treated']['survival_prob'], t=self.time_horizon
            )
            comparison['ipw']['rmst_control'] = restricted_mean_survival_time(
                ipw_curves['control']['timeline'], ipw_curves['control']['survival_prob'], t=self.time_horizon
            )
            
            comparison['aipw']['rmst_treated'] = restricted_mean_survival_time(
                aipw_curves['treated']['timeline'], aipw_curves['treated']['survival_prob'], t=self.time_horizon
            )
            comparison['aipw']['rmst_control'] = restricted_mean_survival_time(
                aipw_curves['control']['timeline'], aipw_curves['control']['survival_prob'], t=self.time_horizon
            )

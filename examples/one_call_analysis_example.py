...
def create_sample_marketing_data():
    ...
    revenue = (
        base_revenue + treatment_effect * email_campaign + np.random.normal(0, 50, n)
    )
    ...
    print(
        f"📧 Email campaign sent to {email_campaign.sum():,} customers ({email_campaign.mean():.1%})"
    )
...
def demonstrate_one_line_analysis():
    ...
    print(
        f"   95% Confidence Interval: [${effect.ate_ci_lower:.2f}, ${effect.ate_ci_upper:.2f}]"
    )
    print(
        f"   Statistical Significance: {'Yes' if effect.p_value < 0.05 else 'No'} (p = {effect.p_value:.4f})"
    )
    ...
    print(
        "   Contains: Executive Summary, Diagnostics, Recommendations, Sensitivity Analysis"
    )
...
def demonstrate_custom_analysis():
    ...
    print("   Method: AIPW (Doubly Robust)")
    ...
    print("   Template: Full (Executive + Technical)")
    print("   Includes: Sensitivity Analysis, Diagnostics, Business Recommendations")
...
def demonstrate_file_workflow():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        analysis.report(
            output_path=str(report_file), title="Email Campaign Analysis Report"
        )
        ...
        print("\n📋 Report Preview:")
        print(
            f"   Contains 'Executive Summary': {'✅' if 'Executive Summary' in content else '❌'}"
        )
        print(
            f"   Contains 'Treatment Effect': {'✅' if 'Treatment Effect' in content else '❌'}"
        )
        print(
            f"   Contains 'Recommendations': {'✅' if 'Recommendations' in content else '❌'}"
        )
...
def demonstrate_method_comparison():
    ...
    for method, result in results.items():
        ...
        print(
            f"{actual_method:<15} ${result['ate']:<9.2f} {ci_str:<25} {result['p_value']:<10.4f}"
        )
    ...
    print(
        f"   Coefficient of Variation: {np.std(ate_values) / np.mean(ate_values):.2%}"
    )
